var resolve = {
	options : {
		jsonUrl : 'examples/form/json/',// json文件目录
	...
	bulid_Panel : function(panel) {
		...
		if ("input" == _self.jsonGetVal(panel, "uiType")) {
			...
			_panelDiv.className = "panel-auto " + _self.jsonGetVal(panel, "domClass");
			addClass(_panelDiv,"panel-"+panel["deep"]+"_"+panel["index"]);
			addClass(_panelDiv,"panel-scroll");
			_panelDiv.setAttribute("onclick", _self.jsonGetVal(panel, "onclick"));
			...
			if(panel.type=="card"){
				_panelDiv.className = "mui-card  panel-auto " + _self.jsonGetVal(panel, "domClass");
			}else if(panel.type=="form"){
				_panelDiv.className = "mui-input-group panel-auto " + _self.jsonGetVal(panel, "domClass");
			}else{
				_panelDiv.className = "panel-auto " + _self.jsonGetVal(panel, "domClass");
			}
			addClass(_panelDiv,"panel-"+panel["deep"]+"_"+panel["index"]);
	...
	textLabel : function(tag) {
		...
		if (_self.jsonGetVal(tag, "readonly", false)) {
			$input.setAttribute("readonly", _self.jsonGetVal(tag, "readonly",false));
		}
		...
		$input.setAttribute("placeholder", _self.jsonGetVal(tag,"placeholder"));
	...
	textareaLabel : function(tag) {
		...
		$input.setAttribute("onkeydown", _self.jsonGetVal(tag, "onkeydown"));
		$input.setAttribute("onkeypress", _self.jsonGetVal(tag, "onkeypress"));
		$input.setAttribute("placeholder", _self.jsonGetVal(tag,
						"placeholder"));
		$input.setAttribute("title", _self.jsonGetVal(tag, "labelName"));
		$label.innerHTML = _self.jsonGetVal(tag, "labelName");
		addClass($tag, _self.jsonGetVal(tag, "domClass"));
	...
	inputGroupTag : function(elem, panel) {
		...
		try {
			...
			$labelTag.setAttribute("onclick", _self.jsonGetVal(labelTag,"onclick"));
			$labelTag.innerHTML = _self.jsonGetVal(labelTag, "initValue");
			$labelTag.setAttribute("class", _self.jsonGetVal(labelTag,"domClass"));
			$labelTag.setAttribute("name", _self.jsonGetVal(labelTag,"fieldName"));
			
			var $inputTag = document.createElement("input");
			$inputTag.className = "mui-input-clear";
			$inputTag.setAttribute("placeholder", "请输入");
			
			$labelTag.setAttribute("style", _self.jsonGetVal(labelTag, "animate"));
			$inputTag.setAttribute("style", _self.jsonGetVal(inputTag, "animate"));
			
			$inputTag.setAttribute("name", _self.jsonGetVal(inputTag,"fieldName"));
			$inputTag.setAttribute("dataType", _self.jsonGetVal(inputTag,"dataType"));
			...
			$inputTag.setAttribute("mustinput", _self.jsonGetVal(inputTag,"mustInput"));
			if (_self.jsonGetVal(inputTag, "readonly", false)) {
				$inputTag.setAttribute("readonly", _self.jsonGetVal(inputTag,"readonly", false));
			}
